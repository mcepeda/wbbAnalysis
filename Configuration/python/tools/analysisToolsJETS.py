...
def jetOverloading(process,src = "selectedPatJets"):
  ...
  # This is the default for everybody 
  process.patOverloadedJets = cms.EDProducer('PATJetOverloader',
                                        src = cms.InputTag(src)
  )                                        


  # This is stuff added for W+Jets analysis (will be redundant with the CrossCleaning modules)
  process.cleanPatJets = cms.EDProducer("PATJetCleaner",
    src = cms.InputTag(src),

    # preselection (any string-based cut on pat::Jet)  --> Loose Jet ID
...
#           algorithm = cms.string("byDeltaR"),
#           preselection        = cms.string("pt>15&&tauID('decayModeFinding')&&tauID('byLooseIsolation')"),
#           deltaR              = cms.double(0.3),
#           checkRecoComponents = cms.bool(False),
#           pairCut             = cms.string(""),
#           requireNoOverlaps   = cms.bool(True),
#      )
  ...
  process.patJetOverloaded2 = cms.EDProducer("PATJetOverloader", 
                                          src = cms.InputTag("cleanPatJets"),
                                          srcRho = cms.InputTag("kt6PFJets","rho")
  )

  process.patRhoJets = cms.EDProducer("JetRhoOverloader",
                                          src = cms.InputTag("patJetOverloaded2"),
                                          srcRho = cms.InputTag("kt6PFJets","rho")
  )
  ...
  process.patJetsForAnalysis = cms.EDProducer('PATMuonInJetEmbedder',    
                                        src = cms.InputTag("patSSVEmbeded"),
                                              srcVertex     = cms.InputTag("primaryVertexFilter"),
 )

 # CAREFUL: Use only Michalis' default jets (patOverloadedJets) unless you are sure that you are happy with this cleaning 

  process.jetOverloading = cms.Sequence(process.patOverloadedJets*process.cleanPatJets*process.patJetOverloaded2*process.patRhoJets*process.patSSVEmbeded*process.patJetsForAnalysis)
...
def electronOverloading(process,isdata,src):
  ...
  process.electronOverloading=cms.Sequence(process.electronsWP70+process.electronsWP80+process.electronsWP90+process.electronsWP95+process.patRhoElectron+process.mvaElectrons+process.convRejElectrons)
