...
################################################################################
    ...
    def addCandidateMETModule(self,moduleName,moduleType, srcLeptons,srcMET,srcJets,min = 1,max=9999,text = '',genParticles='genParticles'):
               dicand  = cms.EDProducer(moduleType)
    ...
    def addSmearing(self,taus,muons,electrons,jets,mets,metKind,mpost=''):
...
#          self.sequence*=smearedTausID
          ...
          smearedMET = cms.EDProducer('METRecalculator',
				      ...
				      metCorrection = cms.string(metKind)
                                      )
          pyModule = sys.modules[self.pyModuleName[0]]
          if pyModule is None:
              raise ValueError("'pyModuleName' Parameter invalid")
