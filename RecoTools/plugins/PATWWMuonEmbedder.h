...
class PATWWMuonEmbedder : public edm::EDProducer {
...

 {
  ...
  virtual void produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
  {
	...
	double d0=100.;
	double dz=100.0;
        double dxyBeamSpot=-222, dzBeamSpot=-222;
	...
	if(muon.globalTrack().isNonnull()){
              dxyBeamSpot=muon.globalTrack()->dxy(beamSpotHandle->position());
              dzBeamSpot=muon.globalTrack()->dz(beamSpotHandle->position());
	}
...
// 	  if(!muon::isGoodMuon(muon,muon::GlobalMuonPromptTight)) {pass=false; }
	...
	if(pass)
	  ...
	  muon.addUserFloat("isWWMuon",0.0);
        ...
        // Redundant? 
          muon.addUserFloat("dxyBeamSpot",dxyBeamSpot);
          muon.addUserFloat("dzBeamSpot",dzBeamSpot);
          muon.addUserFloat("dxyVtx",d0);
          muon.addUserFloat("dzVtx",dz);
