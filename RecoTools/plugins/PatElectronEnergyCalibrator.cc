...
//	float test=electron.superCluster()->rawEnergy();
	// This is just while I figure out what happens:
//	if(electron.userFloat("rawEnergy")==0) r9=0;
...
void PatElectronEnergyCalibrator::computeNewEnergy
 ( const pat::Electron & electron, float r9, int run)
 {
  ...
  if (dataset_=="Summer11"||dataset_=="ReReco") { // values from https://indico.cern.ch/conferenceDisplay.py?confId=146386
	...
	std::cout<<"I am here!!  ---> dsigMC="<<dsigMC<<" --> EB"<<electron.isEB()<<"   SC"<<electron.superCluster()->eta()<<"   r9="<<r9<<std::endl;
    ...
    if (electron.isEE() && fabs(electron.superCluster()->eta())>=2 && r9>=0.94) dsigMC = 0.0293;
	 std::cout<<"And now here!!  ---> dsigMC="<<dsigMC<<std::endl;
  ...
  if (isMC_) {
    ...
    if (debug_) std::cout << "[PatElectronEnergyCalibrator] dsigMC " << dsigMC << std::endl;
    if (debug_) std::cout << "[PatElectronEnergyCalibrator] corrMC " << corrMC << std::endl;
