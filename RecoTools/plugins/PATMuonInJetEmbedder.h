    ...
    if(iEvent.getByLabel(src_,cands))
	  ...
	  for(int i=0; i<nConst; i++){
	        ...
	        if (!muref->isTrackerMuon()) continue;
          	...
          	double dxy = 100; double dz=100; 
		if(vertexHandle->size()>=1){
                dxy=muref->globalTrack()->dxy(vertexHandle->at(0).position());
          	dz = muref->globalTrack()->dz(vertexHandle->at(0).position());
		}
		...
		if(fabs(dz)>0.2) continue;
        	...
        	if (nMatches<2) continue;
					...
					MuonInJetDXY=dxy;
					MuonInJetDZ=dz;
					MuonInJetDXYERR= muref->globalTrack()->dxyError();
                                        MuonInJetDZERR= muref->globalTrack()->dzError();
