...
#include "UWAnalysis/RecoTools/plugins/CalibratedPatElectronProducerV2.h"
...
CalibratedPatElectronProducerV2::CalibratedPatElectronProducerV2( const edm::ParameterSet & cfg )
// : PatElectronBaseProducer(cfg)
 {
  ...
  if (isMC&&(dataset!="Summer11"&&dataset!="Fall11"&&dataset!="Summer12"&&dataset!="Summer12_HCP2012"))
   { throw cms::Exception("CalibratedPatElectronProducerV2|ConfigError")<<"Unknown MC dataset" ; }
  if (!isMC&&(dataset!="Prompt"&&dataset!="ReReco"&&dataset!="Jan16ReReco"&&dataset!="Prompt2012"&&dataset!="ICHEP2012"&&dataset!="HCP2012"))
   { throw cms::Exception("CalibratedPatElectronProducerV2|ConfigError")<<"Unknown Data dataset" ; }
  cout << "[CalibratedPatElectronProducerV2] Correcting scale for dataset " << dataset << endl;

 }
 
CalibratedPatElectronProducerV2::~CalibratedPatElectronProducerV2()
 {}

void CalibratedPatElectronProducerV2::produce( edm::Event & event, const edm::EventSetup & setup )
 {
     ...
     ++ele )
   {
     if (energyMeasurementType == 0) {
       ...
       if (ele->core()->ecalDrivenSeed()) {
         ...
         //std::cout << "[CalibratedPatElectronProducerV2] is tracker driven only!!" << std::endl;
       }
     } else {
...
DEFINE_FWK_MODULE(CalibratedPatElectronProducerV2);
